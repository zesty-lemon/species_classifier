...
def train_fine_tune_model(model, train_loader, val_loader, device, device_name, epochs=5, lr=0.01, name="Model",
                          patience=7):
    """
    Fine-tuning training loop with cosine annealing LR and early stopping. Returns all metrics for comparison.
    """
    print("------ BEGIN Training Model ------")
    model = model.to(device)
    criterion = nn.CrossEntropyLoss()
    ...
    scaler = torch.amp.GradScaler(enabled=(device_name == 'cuda'))
    ...
    for epoch in range(epochs):
        ...
        print(f"  Epoch [{epoch + 1}/{epochs}] | Train Loss: {train_loss:.4f} | Train Acc: {train_acc:.2f}% | Train Top-5: {train_top5_acc:.2f}% | Val Loss: {epoch_val_loss:.4f} | Val Acc: {epoch_val_acc:.2f}% | Val Top-5: {epoch_val_top5_acc:.2f}% | LR: {current_lr:.6f}")
